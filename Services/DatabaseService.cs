...
namespace SkillSwap.Services
{
    public class DatabaseService
    {
        ...
        public async Task<int> GuardarAnuncioAsync(int usuarioId, int postId)
        {
            await InitAsync();
            var existe = await _database!.Table<SavedPost>()
                .FirstOrDefaultAsync(s => s.UsuarioId == usuarioId && s.PostId == postId);
            if (existe != null) return 0; // ya estaba guardado
            return await _database!.InsertAsync(new SavedPost { UsuarioId = usuarioId, PostId = postId });
        }
        ...
        public async Task<List<Post>> ObtenerAnunciosGuardadosAsync(int usuarioId)
        {
            ...
            var guardados = await _database!.Table<SavedPost>()
                .Where(s => s.UsuarioId == usuarioId).ToListAsync();
            var postIds = guardados.Select(s => s.PostId).ToHashSet();
            var todos = await GetAllPostsAsync();
            return todos.Where(p => postIds.Contains(p.Id)).ToList();
        }

        public async Task<int> EliminarAnuncioGuardadoAsync(int usuarioId, int postId)
        {
            await InitAsync();
            var item = await _database!.Table<SavedPost>()
                .FirstOrDefaultAsync(s => s.UsuarioId == usuarioId && s.PostId == postId);
